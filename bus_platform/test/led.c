...
struct led_resource {
    ...
    void __iomem *va_DR; // Data Register 数据寄存器
    void __iomem *va_DDR; // Data Direction Register 数据方向寄存器
};
...
/*字符设备操作函数集，write函数*/
static ssize_t led_chr_dev_write(
    struct file *filp,
...
static const struct of_device_id led_ids[] = { { .compatible =
                                                     "fire,led_test" },
                                               { /* sentinel */ } };
